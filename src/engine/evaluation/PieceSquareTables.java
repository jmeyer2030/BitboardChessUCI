package engine.evaluation;

import board.Position;

import static engine.evaluation.KingSafety.kingSafetyEvaluation;

public class PieceSquareTables {

    // Piece values (pawn, knight, bishop, rook, queen, king)
    public static final int[] mgValue = {82, 347, 365, 477, 1025, 0};
    public static final int[] egValue = {94, 281, 297, 512, 936, 0};

    /*
    * Turn the formatter off to keep arrays aligned
    * @formatter:off
    */

    // Flip is used to mirror pst's for black piece values
    public static final int[] flip = {
            56, 57, 58, 59, 60, 61, 62, 63,
            48, 49, 50, 51, 52, 53, 54, 55,
            40, 41, 42, 43, 44, 45, 46, 47,
            32, 33, 34, 35, 36, 37, 38, 39,
            24, 25, 26, 27, 28, 29, 30, 31,
            16, 17, 18, 19, 20, 21, 22, 23,
            8,   9, 10, 11, 12, 13, 14, 15,
            0,   1,  2,  3,  4,  5,  6,  7,
            };

    // Tables
    public static final int[] mgPawnTable = {
              0,   0,   0,   0,   0,   0,  0,   0,
            -35,  -1, -20, -23, -25,  24, 38, -22,
            -26,  -4,  -4, -10,  -5,   3, 33, -12,
            -27,  -2,  -5,  16,  19,   6, 10, -25,
            -14,  13,   6,  21,  23,  12, 17, -23,
             -6,   7,  26,  31,  65,  56, 25, -20,
             98, 134,  61,  95,  68, 126, 34, -11,
              0,   0,   0,   0,   0,   0,  0,   0,
            };

    public static final int[] egPawnTable = {
            0, 0, 0, 0, 0, 0, 0, 0,
            -10, -10, -10, -10, -10, -10, -10, -10,
            5, 5, 5, 5, 5, 5, 5, 5,
            35, 10, 5, 5, 5, 5, 10, 35,
            100, 50, 50, 50, 50, 50, 50, 100,
            200, 100, 100, 100, 100, 100, 100, 200,
            350, 250, 200, 200, 200, 200, 250, 350,
            0, 0, 0, 0, 0, 0, 0, 0,
            };

    public static final int[] mgKnightTable = {
            -105, -21, -58, -33, -17, -28, -19, -23,
            -29, -53, -12, -3, -1, 18, -14, -19,
            -23, -9, 12, 10, 19, 17, 25, -16,
            -13, 4, 16, 13, 28, 19, 21, -8,
            -9, 17, 19, 53, 37, 69, 18, 22,
            -47, 60, 37, 65, 84, 129, 73, 44,
            -73, -41, 72, 36, 23, 62, 7, -17,
            -167, -89, -34, -49, 61, -97, -15, -107,
            };

    public static final int[] egKnightTable = {
            -29, -51, -23, -15, -22, -18, -50, -64,
            -42, -20, -10, -5, -2, -20, -23, -44,
            -23, -3, -1, 15, 10, -3, -20, -22,
            -18, -6, 16, 25, 16, 17, 4, -18,
            -17, 3, 22, 22, 22, 11, 8, -18,
            -24, -20, 10, 9, -1, -9, -19, -41,
            -25, -8, -25, -2, -9, -25, -24, -52,
            -58, -38, -13, -28, -31, -27, -63, -99,
            };

    public static final int[] mgBishopTable = {
            -33, -3, -24, -21, -13, -22, -39, -21,
            4, 15, 16, 0, 7, 21, 33, 1,
            0, 15, 15, 15, 14, 27, 18, 10,
            -6, 13, 13, 26, 34, 12, 10, 4,
            -4, 5, 19, 50, 37, 37, 7, -2,
            -16, 37, 43, 40, 35, 50, 37, -2,
            -26, 16, -18, -13, 30, 59, 18, -47,
            -29, 4, -82, -37, -25, -42, 7, -8,
            };

    public static final int[] egBishopTable = {
            -23, -9, -23, -5, -9, -16, -5, -17,
            -14, -18, -7, -1, 4, -9, -15, -27,
            -12, -3, 8, 10, 13, 3, -7, -15,
            -6, 3, 13, 19, 7, 10, -3, -9,
            -3, 9, 12, 9, 14, 10, 3, 2,
            2, -8, 0, -1, -2, 6, 0, 4,
            -8, -4, 7, -12, -3, -13, -4, -14,
            -14, -21, -11, -8, -7, -9, -17, -24,
            };

    public static final int[] mgRookTable = {
            -19, -13, 1, 17, 16, 7, -37, -26,
            -44, -16, -20, -9, -1, 11, -6, -71,
            -45, -25, -16, -17, 3, 0, -5, -33,
            -36, -26, -12, -1, 9, -7, 6, -23,
            -24, -11, 7, 26, 24, 35, -8, -20,
            -5, 19, 26, 36, 17, 45, 61, 16,
            27, 32, 58, 62, 80, 67, 26, 44,
            32, 42, 32, 51, 63, 9, 31, 43,
            };

    public static final int[] egRookTable = {
            -9, 2, 3, -1, -5, -13, 4, -20,
            -6, -6, 0, 2, -9, -9, -11, -3,
            -4, 0, -5, -1, -7, -12, -8, -16,
            3, 5, 8, 4, -5, -6, -8, -11,
            4, 3, 13, 1, 2, 1, -1, 2,
            7, 7, 7, 5, 4, -3, -5, -3,
            11, 13, 13, 11, -3, 3, 8, 3,
            13, 10, 18, 15, 12, 12, 8, 5,
            };

    public static final int[] mgQueenTable = {
            -1, -18, -9, 10, -15, -25, -31, -50,
            -35, -8, 11, 2, 8, 15, -3, 1,
            -14, 2, -11, -2, -5, 2, 14, 5,
            -9, -26, -9, -10, -2, -4, 3, -3,
            -27, -27, -16, -16, -1, 17, -2, 1,
            -13, -17, 7, 8, 29, 56, 47, 57,
            -24, -39, -5, 1, -16, 57, 28, 54,
            -28, 0, 29, 12, 59, 44, 43, 45,
            };

    public static final int[] egQueenTable = {
            -33, -28, -22, -43, -5, -32, -20, -41,
            -22, -23, -30, -16, -16, -23, -36, -32,
            -16, -27, 15, 6, 9, 17, 10, 5,
            -18, 28, 19, 47, 31, 34, 39, 23,
            3, 22, 24, 45, 57, 40, 57, 36,
            -20, 6, 9, 49, 47, 35, 19, 9,
            -17, 20, 32, 41, 58, 25, 30, 0,
            -9, 22, 22, 27, 27, 19, 10, 20,
            };

    public static final int[] mgKingTable = {
            -15, 36, 12, -54, 8, -28, 34, 14,
            1, 7, -8, -64, -43, -16, 9, 8,
            -14, -14, -22, -46, -44, -30, -15, -27,
            -49, -1, -27, -39, -46, -44, -33, -51,
            -17, -20, -12, -27, -30, -25, -14, -36,
            -9, 24, 2, -16, -20, 6, 22, -22,
            29, -1, -20, -7, -8, -4, -38, -29,
            -65, 23, 16, -15, -56, -34, 2, 13,
            };

    public static final int[] egKingTable = {
            -70, -60, -50, -40, -40, -50, -60, -70,
            -60, -50, -30, -20, -20, -30, -50, -60,
            -50, -30,   0,   0,   0,   0, -30, -50,
            -40, -20,   0,   0,   0,   0, -20, -40,
            -40, -20,   0,   0,   0,   0, -20, -40,
            -50, -30,   0,   0,   0,   0, -30, -50,
            -60, -50, -30, -20, -20, -30, -50, -60,
            -70, -60, -50, -40, -40, -50, -60, -70,
            };

    /*
    * Turn formatter back on
    * @formatter:on
    */

    public static int[] gameStageInc = {0, 1, 1, 2, 4, 0};
    //public static int[] gameStageInc = {0,0,1,1,1,1,2,2,4,4,0,0};

    public static final int[][] mgTables = {mgPawnTable, mgKnightTable, mgBishopTable, mgRookTable, mgQueenTable, mgKingTable};
    public static final int[][] egTables = {egPawnTable, egKnightTable, egBishopTable, egRookTable, egQueenTable, egKingTable};

    public static final int[][][] pieceTables = {mgTables, egTables}; // [mg/eg][piece][square]
    public static final int[][] pieceValues = {mgValue, egValue};

    public static int computeMGScore(Position position) {
        int mgScore = 0;
        for (int color = 0; color <= 1; color++) { //iterate for each color
            for (int piece = 0; piece < 6; piece++) { //iterate for each pieceType
                long pieces = position.pieceColors[color] & position.pieces[piece];
                while (pieces != 0) {
                    int square = Long.numberOfTrailingZeros(pieces);
                    pieces &= (pieces - 1);

                    if (color == 0) {//if white
                        mgScore += pieceValues[0][piece] + pieceTables[0][piece][square];
                    } else {
                        mgScore -= (pieceValues[0][piece] + pieceTables[0][piece][flip[square]]);
                    }
                }
            }
        }

        return mgScore;

    }

    public static int computeEGScore(Position position) {
        int egScore = 0;
        for (int color = 0; color <= 1; color++) { //iterate for each color
            for (int piece = 0; piece < 6; piece++) { //iterate for each pieceType
                long pieces = position.pieceColors[color] & position.pieces[piece];
                while (pieces != 0) {
                    int square = Long.numberOfTrailingZeros(pieces);
                    pieces &= (pieces - 1);

                    if (color == 0) {//if white
                        egScore += pieceValues[1][piece] + pieceTables[1][piece][square];
                    } else {
                        egScore -= (pieceValues[1][piece] + pieceTables[1][piece][flip[square]]);
                    }
                }
            }
        }

        return egScore;
    }

    public static int computeGamePhase(Position position) {
        int gamePhase = 0;

        for (int color = 0; color <= 1; color++) { //iterate for each color
            for (int piece = 0; piece < 6; piece++) { //iterate for each pieceType
                long pieces = position.pieceColors[color] & position.pieces[piece];
                while (pieces != 0) {
                    pieces &= (pieces - 1);

                    gamePhase += gameStageInc[piece];
                }
            }
        }

        return gamePhase;
    }


    public static int evaluatePosition(Position position) {
        int mgScore = 0;
        int egScore = 0;
        int gamePhase = 0;

        for (int i = 0; i <= 1; i++) { //iterate for each color
            for (int j = 0; j < 5; j++) { //iterate for each pieceType
                long pieces = position.pieceColors[i] & position.pieces[j];
                while (pieces != 0) {
                    int square = Long.numberOfTrailingZeros(pieces);
                    gamePhase += gameStageInc[j];
                    if (i == 0) {//if white
                        mgScore += pieceValues[0][j] + pieceTables[0][j][square];
                        egScore += pieceValues[1][j] + pieceTables[1][j][square];
                    } else {
                        mgScore -= (pieceValues[0][j] + pieceTables[0][j][flip[square]]);
                        egScore -= (pieceValues[1][j] + pieceTables[1][j][flip[square]]);
                    }

                    pieces &= pieces - 1;
                }
            }
        }

        mgScore += kingSafetyEvaluation(position);

        int mgPhase = gamePhase;

        if (mgPhase > 24) {
            mgPhase = 24;
        }

        int egPhase = 24 - mgPhase;

        int evaluation = (mgScore * mgPhase + egScore * egPhase) / 24;

        evaluation += Mobility.mobility(position, mgPhase, egPhase);

        return evaluation;
    }
}
